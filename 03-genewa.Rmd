...
between cases and controls (Figure \@ref(fig:pcs)).
...
method_summary <- rbind(
  c('dmGWAS', 'GWAS', 'Genes', 'No', '', '1', 'Summary', '-log~10~(P)', '[@jia_dmgwas:_2011]'),
  c('heinz', 'Omics', 'Genes', 'Yes', '', '1', 'Summary', 'BUM', '[@dittrich_identifying_2008]'),
  c('HotNet2', 'Omics', 'Genes', 'Yes', 'Modular', '$\\ge$ 1', 'Summary', 'Local FDR', '[@leiserson_pan-cancer_2015]'),
  c('LEAN', 'Omics', 'Genes', 'Yes', 'Star', '$\\ge$ 1', 'Summary', '-log~10~(P)', '[@gwinner_network-based_2016]'),
  c('SConES', 'GWAS', 'SNPs', 'Yes', 'Modular', '$\\ge$ 1', 'Genotypes', '$\\chi^2$', '[@azencott_efficient_2013]'),
  c('SigMod', 'GWAS', 'Genes', 'Yes', 'Modular', '1', 'Summary', '-log~10~(P)', '[@liu_sigmod:_2017]')
)

colnames(method_summary) <- c('Method', 'Field', 'Nodes', 'Exhaustive', 'Solution', 'Components', 'Input', 'Scoring', 'Reference')

kableExtra::footnote(knitr::kable(method_summary, 
                     ...
                     general = '/Field/: field in which the algorithm was developed. /Nodes/, the type of network, either gene (protein-protein interaction network usually) or a SNP network. /Exhaustive/: whether all the possible solutions given the selected hyperparameters are explored. /Solution/: additional properties are enforced on the solution subnetwork, other than being dense in high scores and connected. /Components/: number of connected subnetworks in the solution. /Input/: genotype data or GWAS summary statistics. /Scoring/: how SNP/gene P-values are transformed into node scores.', 
...
solution_summary <- read.table(text = "
                  Network SNPs Edges Subnetworks $\\overline{\\mbox{Betweenness}}$ $\\hat{\\mbox{P}}$~SNP~
                  GS 197083 197060 NA 2.03*(10^7) 0.49
                  'SConES GS' 1590 1585 5 2.52*(10^7) 0.023
                  GM 197083 6442446 NA 3.99*(10^6) 0.49
                  'SConES GM' 1692 177611 5 4.40*(10^6) 0.055
                  GI 197083 28733720 NA 1.46*(10^6) 0.49
...
```{r manhattan-solutions, echo=FALSE, fig.cap='(ref:fig-manhattan-solutions-caption)'}
...
```{r disgenet-comparison, echo=FALSE, fig.cap='(ref:fig-disgenet-comparison-caption)'}
...
```{r bcac-comparison, echo=FALSE, fig.cap="(ref:fig-bcac-comparison-caption)"}
...
```{r consensus-stats, echo=FALSE, fig.cap='(ref:fig-consensus-stats-caption)', out.height='70%'}
...
```{r benchmark, echo=FALSE, fig.cap='(ref:fig-benchmark-caption)'}
...
(ref:fig-consensus-caption) Consensus subnetwork on GENESIS (Section \@ref(methods:consensus)). Each node is represented by a pie chart, which accounts the methods that selected it. The labeled genes have a VEGAS2v2 P-value \< 0.001 and/or are known breast cancer susceptibility genes (colored in pink).
...
more than two of them (Figure
\@ref(fig:consensus-stats)A). Encouragingly, the more methods
selected a gene, the higher its association was (Figure
