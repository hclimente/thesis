...
Biological systems are the results of complex networks which are increasingly better understood. Such networks often are built around genes, making them the functional units of analysis. Hence, understanding epistatic interactions at a gene level is essential to understand the biological mechanisms of these diseases. Unfortunately, genome-wide interaction association studies (GWAIS) involve many statistical challenges. This project is based on three main procedures to analyze the conversion of SNP-based statistical interaction signals into a gene-based network through the integration of biological knowledge. First, we investigated the use of known gene-gene interactions to focus on the most promising gene-pairs. Second, we looked into several functional SNP-to-gene mappings to map SNPs to these gene interactions, including eQTL and chromatin-based, and filtered the GWAIS data accordingly. Third, we explored the integration of SNP-pairs statistics into gene-pairs statistics using the adaptive truncated product method (ATPM). ATPM does not require parameters, is fast to compute, and does not require a known target distribution. We applied the different variants of these three steps on a GWAS inflammatory bowel disease (IBD) dataset, and compared the results. Overall, knowledge integration decreases the number of tests performed, reducing computing time and boosting statistical power, and provides interpretability. We validate through permutation analysis that type I error is under control. The different protocols produce different results which, albeit overlapping with the known biology of IBD, also highlight different implicated mechanisms. Using eQTLs results in more interactions than not using any filter and physically mapping a SNP to a gene, albeit less significant gene-pairs. This pipeline not only detects interactions at the gene level, but also gives a global visualization that identifies neighbors, hubs and highly connected substructures.
\\end{chapabstract}
\\vskip 0.2in
