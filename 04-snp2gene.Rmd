...
*Diane Duroux, Héctor Climente-González, Lars Wienbrandt, David Ellinghaus, Chloe-Agathe Azencott, Kristel Van Steen*

## Introduction

Genome-wide association studies (GWAS) have identified over 70 000
genetic variants associated with complex traits
[@buniello_nhgri-ebi_2019]. However, often these variants altogether do
not explain the whole genetic architecture of diseases. Possible explanations 
include a large number of common
variants with small effects, rare variants with large effects not
covered in GWAS, unaccounted gene-environment interactions, and genetic
interactions [@manolio_finding_2009]. This project deals with the latter,
epistasis (described in Section \@ref(intro-epistasis)). To date, few 
replicable, functional conclusions have been obtained on
Genome-Wide Association Interaction Studies (GWAIS). In this regard, going
from SNP to gene epistasis helps converting statistical findings into
biological hypotheses [@lehne2011snps] and facilitates the functional
interpretability of findings [@Jorgenson]. In such gene-level tests, the SNPs
mapped to a gene are jointly considered as a set. Aggregation of such
SNP statistics into gene statistics is likely to increase the
statistical power when dealing with complex diseases [@wu2010novel]. Yet, 
specific gene-gene interactions have rarely been identified. This is largely 
due to the statistical and biological challenges described in Sections 
\@ref(intro-gwas-challenges) and \@ref(gwais), mainly low power, and the 
difficulty to link tag SNPs to the risk SNP and to genes/functions.
...
several genes, we investigated the use of current knowledge on gene-gene
...
product method to estimate gene-pairs significance. Protocols and their
associated outputs are compared through networks.
non-additive genetic effects in diseases. 

## Materials and methods

### Dataset and initial quality control

We investigated the IIBDGC dataset [@ellinghaus2016analysis], described in 
Section \@ref(iibdgc-dataset). Its large sample size helps to overcome the 
issue of reduced statistical power due, for example, to multiple testing and LD. 
We performed a quality control as in @ellinghaus2016analysis, reducing the 
number of SNPs from 196 524 to 130 071.

The IIBDGC dataset aggregates different cohorts, and hence contains
...
detect epistasis (Section \@ref(methods:snp-epistasis)). In consequence, we had to adjust the
...
$\pm$ 10kb. The eQTL mapping uses eQTLs, loci that are significantly
associated to the expression of a gene. We obtained eQTL information
from GTEx [@gtex_consortium_genetic_2017], a project that genotype
multiple human subjects, and extracts expression information from their
different tissues. We mapped an eQTL SNP to its target gene when the
...
around the latter's promoter (250bp upstream and 500bp downstream from
...
as interaction pairs, pathways and ontological categories. Biofilter has
become popular to reduce burden of tests and hopefully increase
interpretability. Notably, it does not use trait information.
...
built the four datasets enumerated above: one without any filter (*no
filter*); and one for each SNP to gene mapping (*physical*, *eQTL*,
*chromatin*). To reduce the number of statistical tests to the minimum,
only the SNPs involved in at least one SNP-pair model were included in
the respective analyses. It is worth pointing out that SNP-pair models
...
keeps the number of interactions in the network under control.

#### Post-filtering quality control

Additional quality controls are performed on each of the four generated
...
considered for analysis. Also, we pruned SNPs that are in linkage equilibrium 
...
We used PLINK 1.9 to detect epistasic association with the phenotype.
Specifically, we performed a linear regression on the adjusted phenotypes
with the option `–epistasis` to study the association of all the
possible SNP pairs:
...
evaluate if $\beta_3 \neq 0$. Crucially, PLINK only returns SNP-pairs
...
statistical tests. The exception was the *Standard* process, which only
underwent LD pruning, but no Biofilter filtering.
...
Next we converted SNP-level epistasis into gene-level epistasis. For
this purpose, first significant SNP pairs were converted into gene-gene
interactions for each dataset. We associated SNP-pairs to gene-pairs
using both FUMA and Biofilter, using the same procedure described in
\@ref(methods:prefiltering). The exception was the *Standard*
...
TPM is interesting since it is fast to compute and we do not have
...
datasets_snps <- rbind(
      ...
      c('# SNP-gene mappings', NA, 1.7e5, 2.4e6, 4.1e5),
      c('# tests', 7.2e8, 4.6e6, 8.9e7, 2.2e7)
  )
colnames(datasets_snps) <- c('', 'Standard', 'Physical' ,'Chromatin', 'eQTL')

kableExtra::footnote(knitr::kable(datasets_snps, 
                                  caption = 'Properties of the different SNP-gene mappings and the filtered datasets.'), escape = FALSE)
```
...
interactions (64). Physical protocol produced no significant pairs.
...
snp_networks <- rbind(
      ...
      c('Average degree', 2.07, NA, 1.9, 2.78)
  )
colnames(snp_networks) <- c('', 'Standard', 'Physical' ,'Chromatin', 'eQTL')

kableExtra::footnote(knitr::kable(snp_networks, 
                                  caption = 'Properties of the SNP networks from the different datasets.'), escape = FALSE)
```
...
networks, estimating their significance through ARTP (Section 
...
kableExtra::footnote(knitr::kable(gene_networks, 
                                  caption = 'Properties of the gene networks from the different datasets.'), escape = FALSE)
```
...
```{r gene-networks, echo=FALSE, fig.cap='(ref:fig-gene-network-caption)'}
...
In this chapter we explore a protocol for functional filtering for
epistasis detection on an inflammatory bowel disease dataset (Section
\@ref(methods:pipeline)). This is expected to bring two
advantages. The first one is an increase in statistical power. In GWAIS,
the high dimensionality of data requires a conservative approach to
multiple testing and limits the detection of epistasis with low effect
...
*LINC02178*). Hence, Our results stress that considering the 3D structure of the
genome in GWAIS might inform about the susceptibility.

Nonetheless, epistasis detection still requires making many other
...
other algorithms like MBMDR [@calle2010mbmdr], which share these
properties but make different assumptions about epistasis, would have
been suitable as well.

## Future work

In this chapter, we aim at developing a set of guidelines for the detection of 
gene epistasis, with an application to IBD. The final work will include a 
thorough set of recommendations based on our experience, and the results
justifying each of them. Before we reach that point, a few extra experiments 
are required.

**Comparison between mappings.** Our current protocol produces compelling hypotheses,
...
found without filtering out the interactions not in Biofilter; or if they are
a subset of the interactions that could be found by a joint *physical+chromatin+eQTL*
mapping. The answers to such questions are relevant to the community, 
and cannot be answered without isolating their effect from the Biofilter 
interactions. In other words, we should just map the results of a conventional 
GWAIS result, to observe how they differ from a conventional mapping.

**Disease-specific eQTL mapping.** Risk SNPs from GWAS are often located
...
to IBD (intestines and leukocytes). This is a question that we need to study.

**Most appropriate source of interactions.** In this chapter we worked
exclusively on interactions from Biofilter, which compiles multiple databases.
The version of Biofilter which we built contained 37 266 interactions. This
is notably smaller than other gene interaction databases, like STRING 
[@szklarczyk_string_2019] (11 759 455 interactions). Hence changing
databases might result in more, equally interpretable, detected interactions.

**Pathway analysis.** Pathway enrichment analyses can inform about the broader framework 
in which the observed gene epistasis occurs. We plan on adapting the "network neighborhood 
search" procedure from @yip2018network to build appropriate gene sets. In 
summary, given reference biological network, a gene set for a given pair of genes
...
The question of which is the best reference network, already outlined above, 
applies as well to this part of project. 

Another important question is which is the null hypothesis we are 
testing. In this regard, the literature often distinguishes two kinds of test:
...
all the known genes is not an option, as a GWAS experiment surveys all the 
genome unbiasedly, but not necessarily so all the genes. This is specially true
in an array focused on immunogenomics. Hence, I propose computing the gene 
...
association in the gene which we are testing more often. Hence, I suggest
weighting every gene by the number of SNPs that map to it, which should provide 
a conservative null hypothesis.

**Widely-available computational protocol.** The protocol here presented is a
...
networks, SNP- and gene-based respectively. Generating such pipeline
should be a priority in the project, since it would allow us to carry out 
the aforementioned experiments faster.
...
2.5020.11 and by the Walloon Region. H.C-G. was funded by the European
Union's Horizon 2020 research and innovation program under the Marie
Skłodowska-Curie [666003].