...
The contents of this chapter are based on *Héctor Climente-González, Diane Duroux, Lars Wienbrandt, David Ellinghaus, Chloé-Agathe Azencott and Kristel Van Steen*, Boosting interpretability and statistical power in epistasis detection by using prior biological knowledge (manuscript in preparation).
...
Biological systems are the results of complex networks which are increasingly better understood. Such networks often are built around genes, making them the functional units of analysis. Hence, detecting epistatic interactions at the gene level is essential to understanding the biological mechanisms of these diseases. Unfortunately, genome-wide interaction association studies (GWAIS) involve many statistical challenges that make such detection hard. This project is based on three main procedures to analyze the conversion of SNP interactions into a gene-based network through the integration of biological knowledge. First, we investigated the use of known gene-gene interactions to focus on the most promising gene-pairs. Second, we looked into several functional SNP-to-gene mappings to map SNPs to these gene interactions, including eQTL and chromatin-based, and filtered the GWAIS data accordingly. Third, we explored the integration of SNP-pairs statistics into gene-pairs statistics using the adaptive truncated product method (ATPM). ATPM does not require parameters or a known null distribution, and it is fast to compute. We applied different variants of these three steps to a GWAS inflammatory bowel disease (IBD) dataset, and compared the results. Overall, knowledge integration decreases the number of tests performed, reducing computational time and boosting statistical power, and provides interpretability. We validate through a permutation analysis that type I error is under control. The different protocols produce different results which highlight different mechanisms implicated in IBD while, at the same time, overlapping with the known biology of the disease. Using eQTLs results in more interactions than what we call a Standard approach (not using any filter and physically mapping a SNP to a gene), albeit less significant gene-pairs. This pipeline not only detects interactions at the gene level, but also gives a global visualization that identifies neighbors, hubs and highly connected substructures.
\\end{chapabstract}
\\vskip 0.2in
...
$\pm$ 10kb. The eQTL mapping uses eQTLs obtained
